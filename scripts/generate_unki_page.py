...
<main>
  ...
  <section>
    <h2>今年の吉方位 <span class="en">LUCKY DIRECTION</span></h2>
    <div class="card">
      <div class="houi-wrap">
...
// ===== 厄年判定（数え年）=====
// 男性本厄: 25/42/61、女性本厄: 19/33/37/61（両性共通の61含む合算で判定）
// CSVに性別フィールドがないため、両性の厄年を合算した表で判定する
function getYakudoshi(birthYear, today) {{
  const k = today.getFullYear() - birthYear + 1;
  const honYaku  = [19, 25, 33, 37, 42, 61];
  const maeYaku  = [18, 24, 32, 36, 41, 60];
  const atoYaku  = [20, 26, 34, 38, 43, 62];
  if (honYaku.includes(k))  return ["本厄",  `数え年${{k}}歳（最も注意が必要な年）`, true];
  if (maeYaku.includes(k))  return ["前厄",  `数え年${{k}}歳（本厄の前年）`, true];
  if (atoYaku.includes(k))  return ["後厄",  `数え年${{k}}歳（本厄の翌年）`, true];
...
// バイオリズム3軸のうち低調期の数をカウント（臨界日は守り寄りとして加算）
const bioLowCount = [23, 28, 33].filter(p => {{
  const st = bioState(p);
  return st === "low" || st === "critical";
}}).length;
const isMostlyLow = bioLowCount >= 2;

const word = isMostlyLow
  ? (total >= 60 ? "充電期" : total >= 40 ? "静観" : "向かい風")
  : (total >= 80 ? "絶好調" : total >= 65 ? "上昇気流" : total >= 45 ? "凪" : total >= 30 ? "向かい風" : "時化");
document.getElementById("scoreWord").textContent = word;
document.getElementById("scoreDesc").textContent = isMostlyLow
  ? (total >= 60
    ? "守りを固めつつ流れを待つ日。暦の吉日を活かしながら力を温存するとよいとされる。"
    : "バイオリズムの守り期間にあたる日。焦らず充電に徹することが吉とされる。")
  : (total >= 65
    ? "波に乗る一日。心身の充実が感じられやすい日とされる。"
    : total >= 45
      ? "平常運転。バランスの取れた一日とされる。"
...
(function() {{
  ...
  document.getElementById("houiText").innerHTML = `
    ...
    今年の<span class="kichi">吉方位は${{luckyDirs.join("・")}}</span><br>
    <span style="font-size:11px;color:var(--muted)">※九星気学に基づくエンタメ表示です（年盤・年運）</span>`;
}})();
</script>
